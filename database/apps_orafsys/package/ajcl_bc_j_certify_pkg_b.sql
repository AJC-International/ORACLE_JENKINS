CREATE OR REPLACE PACKAGE BODY              ajcl_bc_j_certify_pkg AS
...
--                   ; 
  ...
  PROCEDURE final_report_xlsx_p ( p_company VARCHAR2, -- MB 20251202 -- KANO

                                                    p_row_count OUT VARCHAR2, -- MB 20260109 -- KANO
    ...
    -- Solapa Processed Data
        ...
        SELECT
          ...
          --  gl_date,
            ...
            --dist_code_concatenated distr_account, --MB 20251203 KANO

            b.company||'.'||b.account||'.'||b.department||'.'||b.division||'.'||b.office||'.'||b.product||'.'||b.destination distr_account,            
     ...
     AND a.company = p_company -- MB 20251202 -- KANO
  ...
  --   AND ( a.status IN ('ERROR','REJECTED') OR b.status IN ('ERROR','REJECTED'))
    ...
    as_xlsx.save ( gv_directory_report, gv_report_filename || '_' || p_company || '.' || LOWER(gv_file_format) ); 
    ...
    print_log('p_row_count: '||p_row_count);
  ...
  BEGIN
    ...
    END IF;
      ...
      ELSIF ( gv_file_format = 'XLSX' ) THEN 
        ...
        final_report_xlsx_p ( p_company => '52', -- MB 20251202 -- KANO

                                      p_row_count => v_row_count, -- MB 20260109 -- KANO
        ...
        IF ( v_status != 'S' ) THEN
           ...
           v_error_msg := 'Process failed when trying to generate Certify process report for Company = 52. Please check email with attached log file. (final_report_xlsx_p)';
        ...
        IF v_row_count <> 0 THEN
          ...
          -- MAIL REPORT - COMPANY 52 ----------------------------------------------------------------------------------------------------------

          BEGIN

              ajcl_bc_utils_pkg.send_mail_with_attach ( p_to_mail => gv_email,

                                                        p_subject => gv_bc_ifc || ' Report Company 52 - ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH24:MI:SS') || ' - ' || gv_bc_environment || ' (' || gv_jenkins_build_number || ')',

                                                        p_body => gv_bc_ifc || ' Report.',

                                                        p_type => 'REPORT',

                                                        p_filename => gv_report_filename || '_52', 
        ...
        final_report_xlsx_p ( p_company => '53', -- MB 20251202 -- KANO
        ...
        IF ( v_status != 'S' ) THEN
           ...
           v_error_msg := 'Process failed when trying to generate Certify process report for Company = 53. Please check email with attached log file. (final_report_xlsx_p)';
        ...
        IF v_row_count <> 0 THEN
          ...
          -- MAIL REPORT - COMPANY 53  ----------------------------------------------------------------------------------------------------------

          BEGIN

              ajcl_bc_utils_pkg.send_mail_with_attach ( p_to_mail => gv_email,

                                                        p_subject => gv_bc_ifc || ' Report Company 53 - ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH24:MI:SS') || ' - ' || gv_bc_environment || ' (' || gv_jenkins_build_number || ')',

                                                        p_body => gv_bc_ifc || ' Report.',

                                                        p_type => 'REPORT',

                                                        p_filename => gv_report_filename || '_53', 
