CREATE OR REPLACE PACKAGE BODY ajcl_bc_vendors_to_oracle_pkg IS
...
-- Creation: SBANCHIERI 23-AUG-2023
    ...
    */
    ...
    FOR cv IN c_vendors LOOP
      ...
      BEGIN
        ...
        -- Agregado para Logistics

        ELSIF ( v_pay_group = 'STCARRIER' ) THEN



          v_vendor_type_lookup_code := 'FREIGHT';
        ...
        IF ( v_vendor_id IS NULL ) THEN
          ...
          print_log ( 'Vendor Num: ' || cv.segment1 || ' does not exist. Creating vendor. AP_PO_VENDORS_APIS_PKG.insert_new_vendor' );

          print_log ( 'Vendor Name: ' || cv.vendor_name ||

                   ' | VAT Registration Num: ' || cv.vat_registration_num || 
          ...
          IF ( v_status = 'S' ) THEN
            ...
            -- o si se ejecuta en Oracle no PROD apuntando a Sandbox | Production UAT -- 
        ...
        ELSE -- v_vendor_id IS NOT NULL
          ...
          print_log ( 'Vendor exists. Updating vendor. AP_PO_VENDORS_APIS_PKG.update_vendor' );
          ...
          IF ( v_status = 'S' ) THEN
            ...
            UPDATE po_vendors
               ...
               SET segment1 = cv.segment1,
                   ...
                   vendor_type_lookup_code = v_vendor_type_lookup_code, 
        ...
        print_log ( 'Vendor Site Code: ' || cv.vendor_site_code || 

                 ' | Address1: ' || cv.address_line1 || 

                 ' | Address2: ' || cv.address_line2 || 
        ...
        BEGIN
          ...
          print_log ( 'v_vendor_id: ' || v_vendor_id || ' org_id: ' || cv.org_id || ' | vendor_site_code: ' || cv.vendor_site_code );
        ...
        EXCEPTION
          ...
          WHEN OTHERS THEN
            ...
            print_log ( 'Vendor site does not exist.' );
        ...
        BEGIN
          ...
          print_log ( 'BC Country: ' || cv.country || 
                    ...
                    ' | Oracle Country: ' || v_country || 

                    ' | Oracle Territory Short Name: ' || v_territory_short_name );
        ...
        IF ( v_vendor_site_id IS NULL ) THEN
          ...
          print_log ( 'Site does not exist. Creating site.' );
          ...
          IF ( v_status = 'S' ) THEN
            ...
            print_log ( 'Site created. v_vendor_site_id: ' || v_vendor_site_id );
        ...
        ELSE -- v_vendor_site_id IS NOT NULL
          ...
          print_log ( 'Site exists. Updating site.' );
      ...
      EXCEPTION
        ...
        WHEN OTHERS THEN
          ...
          print_log ( 'General error: ' || v_exception_msg );
  ...
  PROCEDURE vendor_site_contacts_p ( retcode           OUT   NUMBER
    ...
    SELECT company,
      ...
      FROM ajcl_bc_vendor_site_contacts
  ...
  BEGIN
    ...
    print_log ( 'ajcl_bc_vendors_to_oracle_pkg.vendor_site_contacts_p (+)' );     
    ...
    v_url := ajcl_bc_ws_utils_pkg.get_base_ajc_url_f ( p_bc_environment, gv_bc_company_name ) || v_get_api
    ...
    print_log ( 'Se obtienen los contactos creados o modificados y se insertan en la tabla ajcl_bc_vendor_site_contacts.' );



    INSERT 

      INTO ajcl_bc_vendor_site_contacts 
    ...
    FOR cvsc IN c_vendor_site_contacts LOOP
      ...
      BEGIN
        ...
        ELSE
          ...
          UPDATE ajcl_bc_vendor_site_contacts
  ...
  */
  ...
  BEGIN
    ...
    print_log ( 'ajcl_bc_vendors_to_oracle_pkg.create_account_p (+)' );
    ...
    print_log ( 'Checking if bank / branch exists.' ); 
    ...
    BEGIN
      ...
      print_log ( 'Bank / branch exists. v_bank_branch_id: ' || v_bank_branch_id );
    ...
    EXCEPTION
      ...
      WHEN NO_DATA_FOUND THEN
        ...
        print_log ( 'Bank / branch does not exist. Creating.' );
    ...
    IF ( v_bank_branch_id IS NULL ) THEN
      ...
      print_log ( 'Bank / branch created. v_bank_branch_id: ' || v_bank_branch_id );
    ...
    print_log ( 'Account created. v_bank_account_id: ' || v_bank_account_id );   
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      p_error_message := 'Error trying to create bank / branch / account. ' || SQLERRM;
  ...
  PROCEDURE vendor_sites_p ( p_last_bc_processed_date   IN       TIMESTAMP,
    ...
    v_vendor_id                  ajcl_bc_vendors.vendor_id%TYPE;

    v_vendor_name                ajcl_bc_vendors.vendor_name%TYPE;

    v_vendor_site_id             ajcl_bc_vendors.vendor_site_id%TYPE;
  ...
  BEGIN
    ...
    print_log ( 'ajcl_bc_vendors_to_oracle_pkg.vendor_sites_p (+)' );
    ...
    print_log ( 'Getting remit addressses. Inserting into ajcl_bc_vendor_sites.' );



    INSERT 

      INTO ajcl_bc_vendor_sites
    ...
    FOR cvs IN c_vendor_sites LOOP
      ...
      print_log ( 'Vendor No: ' || cvs.vendorNo || 
               ...
               ' | Legacy Vendor Site Name: ' || cvs.legacyVendorSiteName || 

               ' | Address1: ' || cvs.address || 

               ' | Address2: ' || cvs.address2 || 
      ...
      BEGIN
        ...
        BEGIN
          ...
          print_log ( 'Vendor found by segment1. v_vendor_id: ' || v_vendor_id || ' | v_vendor_name: ' || v_vendor_name );
        ...
        EXCEPTION
          ...
          WHEN OTHERS THEN
            ...
            BEGIN
              ...
              print_log ( 'Site already created. Legacy Vendor Site Name was changed in Remit Address. Getting vendor_site_id from custom table to not create a new site.' );
        ...
        IF ( v_vendor_site_id IS NULL ) THEN
          ...
          print_log ( 'Site does not exist. Creating..' );
        ...
        ELSE -- v_vendor_site_id IS NOT NULL
          ...
          print_log ( 'Site exists. Updating..' );
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      print_log ( 'ajcl_bc_vendors_to_oracle_pkg.vendor_sites_p (!). Error: ' || SQLERRM );
  ...
  BEGIN
    ...
    print_log ( 'ajcl_bc_vendors_to_oracle_pkg.vendor_bank_accounts_p (+)' );
    ...
    print_log ( 'The bank accounts of the vendors created or modified in the current run are obtained and inserted into the ajcl_bc_vendor_banks table.' );



    INSERT

      INTO ajcl_bc_vendor_banks
    ...
    FOR cvb IN c_vendor_banks LOOP
      ...
      print_log ( 'Vendor No: ' || cvb.vendorNo || 
               ...
               ' | Legacy Vendor Site Name: ' || cvb.legacyVendorSiteName || 

               ' | Bank Account No: ' || cvb.bankAccountNo );
      ...
      BEGIN
        ...
        BEGIN
          ...
          print_log ( 'Vendor and site found by segment1 and vendor_site_code. v_vendor_id: ' || v_vendor_id || 
        ...
        EXCEPTION
          ...
          WHEN NO_DATA_FOUND THEN
            ...
            BEGIN
              ...
              print_log ( 'Vendor and site found in table ajcl_bc_vendors' || ' | v_vendor_id: ' || v_vendor_id || 
            ...
            EXCEPTION
              ...
              v_error_message := 'Error trying to find vendor and site.';
        ...
        IF ( cvb.Name IS NULL OR cvb.Name = ' ' ) THEN
          ...
          v_error_message := 'Complete Name field in BC (Vendor Bank Account Card)';
        ...
        IF ( cvb.bankAccountNo IS NULL OR cvb.bankAccountNo = ' ' ) THEN
          ...
          v_error_message := 'Complete Bank Account No field in BC (Vendor Bank Account Card)';
        ...
        EXCEPTION
          ...
          WHEN OTHERS THEN
            ...
            v_error_message := 'Error fetching account data ' || cvb.bankAccountNo || ' in table ap_bank_accounts_all.';
        ...
        IF ( v_bank_account_uses_id IS NULL ) THEN
          ...
          print_log ( 'Checking if it has another account assigned for vendor / site, for the same bank. ABA: ' || cvb.transitNo );
          ...
          BEGIN
            ...
            print_log ( 'Account ' || v_inactive_bank_acc_num || ' from the same bank is already assigned to the vendor site.' );
            ...
            print_log ( 'Activating relation. Registros actualizados: ' || SQL%ROWCOUNT );
            ...
            print_log ( 'Account inactivated. Records updated: ' || SQL%ROWCOUNT );
          ...
          print_log ( 'Account is not assigned to the vendor. Assigning..' );
          ...
          EXCEPTION                                                  
            ...
            WHEN OTHERS THEN
              ...
              v_error_message := 'Error trying to assign account to vendor site (ap_bank_account_uses_pkg.insert_row).';
        ...
        ELSE 
          ...
          print_log ( 'Account is already assigned to the vendor site. Updating.' );
          ...
          EXCEPTION
            ...
            WHEN OTHERS THEN
              ...
              v_error_message := 'Error when trying to update the account for the vendor site (ap_bank_account_uses_pkg.update_row).';
      ...
      EXCEPTION
        ...
        WHEN OTHERS THEN
          ...
          print_log ( 'General error: ' || v_error_message );
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      print_log ( 'ajcl_bc_vendors_to_oracle_pkg.vendor_bank_accounts_p (!). Error: ' || SQLERRM );
  ...
  BEGIN
    ...
    print_log( 'ajcl_bc_vendors_to_oracle_pkg.final_report_csv_p (+)' );
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      print_log( 'ajcl_bc_vendors_to_oracle_pkg.final_report_csv_p (!). Error: ' || SQLERRM );
  ...
  BEGIN
    ...
    print_log( 'ajcl_bc_vendors_to_oracle_pkg.final_report_xlsx_p (+)' );
    ...
    ajcl_bc_utils_pkg.create_rep_info_sheet_p ( p_report_title => gv_bc_ifc || ' Report',
                                                ...
                                                p_param_3_title => 'PROCESS_MULTIPLE_SITES',

                                                p_param_3_value => gv_process_multiple_sites );
    ...
    ajcl_bc_utils_pkg.create_sheet_p ( p_sheet_title => 'Vendors',
    ...
    ajcl_bc_utils_pkg.create_sheet_p ( p_sheet_title => 'Sites',
    ...
    ajcl_bc_utils_pkg.create_sheet_p ( p_sheet_title => 'Banks',
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      print_log( 'ajcl_bc_vendors_to_oracle_pkg.final_report_xlsx_p (!). Error: ' || SQLERRM );
  ...
  BEGIN
    ...
    ajcl_bc_utils_pkg.ins_jenkins_concurrent_job_p ( p_request_id => gv_request_id,
                                                     ...
                                                     p_argument3 => p_process_multiple_sites );



    print_log ( 'ajcl_bc_vendors_to_oracle_pkg.main_p (+)' );
    ...
    gv_email := ajcl_bc_utils_pkg.get_emails_f ( 'VENDORS' );
    ...
    gv_process_multiple_sites := p_process_multiple_sites;

    print_log ( 'gv_process_multiple_sites: ' || gv_process_multiple_sites );
    ...
    gv_ap_resp_id := ajcl_bc_utils_pkg.get_company_parameters_p ( p_bc_company_name => gv_bc_company_name,
    ...
    IF ( gv_process_multiple_sites = 'Y' ) THEN
    ...
    IF ( v_status != 'S' ) THEN
      ...
      v_phase := 'vendor_bank_accounts_p';
    ...
    -- Si se actualizó / creo al menos una entidad, se genera excel y se envia mail
  ...
  EXCEPTION
    ...
    WHEN e_default_country THEN
      ...
      v_error_message := 'Error: Profile DEFAULT_COUNTRY not seted up.';

      print_log ( 'ajcl_bc_vendors_to_oracle_pkg.vendors_p (!). ' || v_error_message );
      ...
      RAISE_APPLICATION_ERROR(-20000,'Error: ' || 'Error: Profile DEFAULT_COUNTRY not seted up.' ); 



    WHEN e_vendors THEN

      print_log ( 'ajcl_bc_vendors_to_oracle_pkg.main_p (!). Phase: ' || v_phase || '. Error: ' || v_error_message );
    ...
    WHEN OTHERS THEN
      ...
      print_log ( 'ajcl_bc_vendors_to_oracle_pkg.main_p (!). Error: ' || SQLERRM );
