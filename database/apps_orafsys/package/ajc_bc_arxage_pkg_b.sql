CREATE OR REPLACE PACKAGE BODY ajc_bc_arxage_pkg IS
  ...
  BEGIN 
    ...
    print_log ( 'ajc_bc_arxage_pkg.main_p (+)');
    ...
    print_log ( 'p_reporting_level: ' || p_reporting_level);

    print_log ( 'p_reporting_context: ' || p_reporting_context);

    print_log ( 'p_set_of_books_currency: ' || p_set_of_books_currency);

    print_log ( 'p_chart_of_accounts: ' || p_chart_of_accounts);

    print_log ( 'p_as_of_date: ' || p_as_of_date);

    print_log ( 'p_order_by: ' || p_order_by);

    print_log ( 'p_report_summary: ' || p_report_summary);

    print_log ( 'p_report_format: ' || p_report_format);

    print_log ( 'p_aging_bucket_name: ' || p_aging_bucket_name);

    print_log ( 'p_show_open_credits: ' || p_show_open_credits);

    print_log ( 'p_show_receipts_at_risk: ' || p_show_receipts_at_risk);

    print_log ( 'p_customer_name_low: ' || p_customer_name_low);

    print_log ( 'p_customer_name_high: ' || p_customer_name_high);

    print_log ( 'p_customer_number_low: ' || p_customer_number_low);

    print_log ( 'p_customer_number_high: ' || p_customer_number_high);

    print_log ( 'p_balance_due_low: ' || p_balance_due_low);

    print_log ( 'p_balance_due_high: ' || p_balance_due_high);    

    print_log ( 'p_transaction_type_low: ' || p_transaction_type_low);

    print_log ( 'p_transaction_type_high: ' || p_transaction_type_high);



    print_log ( ' ');

    print_log ( '1. DELETE AJC_BC_ARXAGE');



    DELETE AJC_BC_ARXAGE;
    ...
    populate_table ( p_as_of_date => TO_DATE(SUBSTR(p_as_of_date,1,10),'YYYY/MM/DD'),

                     p_customer_name_low => p_customer_name_low,

                     p_customer_name_high => p_customer_name_high,

                     p_customer_number_low => p_customer_number_low,

                     p_customer_number_high => p_customer_number_high,

                     p_transaction_type_low => p_transaction_type_low,

                     p_transaction_type_high => p_transaction_type_high,

                     p_org_id => p_reporting_context );



    print_log ( ' ');

    print_log ( '3. PRINT REPORT');

    print_report ( p_reporting_level => p_reporting_level,

                   p_reporting_context => p_reporting_context,

                   p_set_of_books_currency => p_set_of_books_currency,
                   ...
                   p_report_summary => p_report_summary,

                   p_report_format => p_report_format,

                   p_aging_bucket_name => p_aging_bucket_name,

                   p_show_open_credits => p_show_open_credits,

                   p_show_receipts_at_risk => p_show_receipts_at_risk,
                   ...
                   p_balance_due_low => p_balance_due_low,

                   p_balance_due_high => p_balance_due_high,
