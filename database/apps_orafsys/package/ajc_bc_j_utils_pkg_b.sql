CREATE OR REPLACE PACKAGE BODY AJC_BC_J_UTILS_PKG IS
...
-- Creation: SBANCHIERI 23-AUG-2023 
  ...
  BEGIN
    ...
    FOR i IN 1 .. v_mail_count LOOP
      ...
      utl_smtp.write_data(v_conn,'From: "AJC BC" <' || gv_from_mail || '>' || utl_tcp.crlf);
  ...
  EXCEPTION
    ...
    WHEN e_parameter_value THEN
      ...
      RAISE_APPLICATION_ERROR(-20000,'AJC_BC_J_UTILS_PKG.purge_log_p - KEEP_X_DAYS only allow numbers.');

    WHEN OTHERS THEN

      RAISE_APPLICATION_ERROR(-20000,'AJC_BC_J_UTILS_PKG.purge_log_p - General Error.');
  ...
  BEGIN 
    ...
    IF ( v_lines_count > 0 ) THEN
      ...
      -- CREATE CSV FROM TABLE AJC_BC_LOGS --------------------------------------------------------------------------------------
  ...
  BEGIN
    ...
    SELECT value
     ...
     WHERE variable = p_variable
       ...
       AND db_name = AJC_BC_J_UTILS_PKG.get_db_name_f;
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      RETURN 'Environment variable ' || p_variable || ' not defined in table AJC_BC_ENVIRONMENT_VARIABLES';
  ...
  BEGIN
    ...
    SELECT DECODE(COUNT(1),0,'N','Y')
     ...
     WHERE db_name = AJC_BC_J_UTILS_PKG.get_db_name_f
