CREATE OR REPLACE PACKAGE              ajcl_bc_j_trv_tpay_pkg AS
  ...
  gv_bc_ifc                  VARCHAR2(200) := 'AJCL BC TRV Triumph Pay Interface';

  gv_bc_ifc_ftp             VARCHAR2(200) := 'AJCL BC TRV Triumph Pay FTP';

  gv_bc_ifc_loader        VARCHAR2(200) := 'AJCL BC TRV Triumph Pay Loader';  

  gv_report_filename         VARCHAR2(100) := 'AJCLBCTRVTPAYINT';
  ...
  gv_support_email         ajcl_bc_integration_emails.emails%TYPE;  
  ...
  PROCEDURE ajcl_trv_validate_tpay_data ( p_status                       OUT   VARCHAR2 );                                           



  PROCEDURE ajcl_trv_tpay_interface (  p_status                       OUT   VARCHAR2 );
